<div align="center">


  <!-- Banner / Logo -->
  <img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=200&section=header&text=Quy%20trình%20-%20QL%20Chất%20Lượng%20Và%20Cải%20Tiến%20Quy%20Trình&fontSize=30" alt="Header" />

  <p align="center">
    <b>Thành viên thực hiện:</b>
	  <span>Lý Kiến Đức - 25410189</span>
  </p>

</div>

---

## 🚀 Link nội dung bài làm:

- 25410189_DoAn_QTQLCL-CTQT.docx

## Tiến độ thực hiện:
- <b>15.07.2026:</b> Phần 1A - Tìm hiểu và viết mô tả bằng lời cho quy trình "Quản lý chất lượng và cải tiến quy trình - FSoft"
- <b>25.07.2026:</b> Phần 1B - Phân tích và xác định yếu tố cần thiết trong quy trình (dựa trên 4 câu hỏi)
- <b>30.07.2026:</b> Phần 2 - Mô hình hóa quy trình (Phân tích và Thiết kế BPMN)
- <b>10.08.2026:</b> Task - Tìm hiểu lý do chọn đề tài "Công ty FPT Software" - Trích dẫn nguồn tham khảo
- <b>10.08.2026:</b> Phần 3 - Phương pháp thực hiện (Dựa trên bằng chứng)
- <b>18.08.2026:</b> Phần 1,2,3 - Chỉnh sửa lại phiên bản cuối và hoàn thành 3 phần này
- <b>23.08.2026:</b> Hoàn thành vẽ lại BPMN hoàn chỉnh (phiên bản cuối) cho Phần 2
- <b>24.08.2026:</b> Phần 4.1 - Phân tích giá trị gia tăng (Hoàn thành)


## Khó khăn khi thực hiện:
- Task: Tìm hiểu lý do chọn đề tài "Công ty FPT Software" - Trích dẫn nguồn tham khảo

  ---> <a href="https://docs.google.com/document/d/1e_txtBy6M4DKooImb5dv8iaTxa0d6ZzrtiJ23opbHQU/edit?tab=t.0">https://docs.google.com/document/d/1e_txtBy6M4DKooImb5dv8iaTxa0d6ZzrtiJ23opbHQU/edit?tab=t.0</a>

## Khó khăn khi thực hiện:
- Chưa được tiếp xúc quy trình này, nên chưa nắm chi tiết về quy trình nghiệp vụ này.
- Tốn thời gian tìm hiểu quy trình này từ các nguồn, tài liệu tham khảo trên Internet.
- Chưa thể xác minh phần quy trình này là chính xác 100% hay không, chỉ là dựa trên các tài liệu tham khảo về CMMI của FSoft được public trên Internet, có thể cho thấy phần mô tả này là tương đối chính xác.

  --> Tài liệu tham khảo đã được đính kèm trong link bên trên

  --> Sẽ tiếp tục phân tích mô hình hóa và các bước tiếp theo, nếu có chỉnh sửa sẽ cập nhật tại đây.
